...
public class ChatService {
    ...
    public MessageDto sendMessage(String senderId, MessageDto messageDto) throws ExecutionException, InterruptedException {
        User sender = userRepository.findByUid(senderId).orElseThrow(() -> new RuntimeException("Sender not found with UID: " + senderId));
        String chatId = getChatId(senderId, messageDto.getReceiverId());
        DocumentReference chatRef = firestore.collection("chats").document(chatId);

        // Ajouter le message à Firestore et obtenir son ID
        DocumentReference messageRef = chatRef.collection("messages").add(messageDto).get();
        String messageId = messageRef.getId();

        // Mettre à jour la conversation
        chatRef.update("lastActivity", FieldValue.serverTimestamp(), "status", "OPEN").get();

        if (sender.getRole() == UserRole.ADMIN || sender.getRole() == UserRole.SELLER) {
            markConversationAsRead(chatId, senderId);
        }
        ...
        userRepository.findByUid(messageDto.getReceiverId()).ifPresent(receiver -> {
            ...
            // **LOGIQUE WHATSAPP CORRIGÉE ET FIABLE**
            String statusUpdateDestination = "/topic/chat/" + chatId + "/status-update";
            String status = isClientOnline ? "delivered" : "sent";
            messagingTemplate.convertAndSend(statusUpdateDestination, Map.of("messageId", messageId, "status", status));

            // On envoie une notification push uniquement si le client est hors ligne
            if (!isClientOnline) {
                pushNotificationService.sendPushNotificationToToken(receiver.getFcmToken(), "New message from " + sender.getFirstName() + " " + sender.getLastName(), messageDto.getContent());
            }

            messagingTemplate.convertAndSendToUser(receiver.getUid(), "/queue/reply", Map.of("chatId", chatId, "from", sender.getFirstName() + " " + sender.getLastName()));
        });
